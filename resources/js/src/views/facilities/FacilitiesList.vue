            ...
            <div class="row">
                <div class="col-sm-12 col-md-6 col-lg-6">
                    <ul class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">{{ $trans('menu.facilities-list') }}</li>
                    </ul>
                </div>
                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-header d-flex align-items-center">
                                <h3 class="h4">{{ $trans('menu.facilities-list') }}</h3>
                            </div>
                            <div class="">
                                <div class="card-body">
                                    ...
                                    <v-table
                                        ...
                                        <tr>
                                            ...
                                            <th class="border-bottom">{{$trans('facility.facility_name')}} </th>
                                            <th class="border-bottom">{{$trans('facility.manager_name')}}</th>
                                            <th class="border-bottom">{{$trans('facility.email')}}</th>
                                            <th class="border-bottom">{{$trans('facility.city')}}</th>
                                            <th class="border-bottom">{{$trans('facility.start_date')}}</th>
                                            <th class="border-bottom">{{$trans('facility.end_date')}}</th>
                                            <th class="border-bottom">{{$trans('facility.status')}}</th>
                                        ...
                                        <tr v-for="row in displayData" :key="row.id" :id="row.id">
                                            ...
                                            <td>{{row.expiry_date | moment("DD/MM/YYYY")}}</td>
                                            ...
                                            <td style="overflow-y:visible !important">
                                                ...
                                                <div aria-labelledby="action_menu" class="dropdown-menu">
                                                    <router-link class="dropdown-item" :to="{name:'facility-edit',params:{idCompany:row.id}}"><span class="fas fa-edit mr-2"></span> {{$trans('main.edit')}}</router-link>
...
export default {
    data () {
        return {
            ...
            totalPages: 0,
            filters: {
                search: { value: '', keys: ['manager_name','manager_mail','website','city'] }
            }
        }
    },
    mounted () {
