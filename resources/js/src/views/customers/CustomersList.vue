            ...
            <div class="row">
                <div class="col-sm-12 col-md-6 col-lg-6">
                    <ul class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">{{$trans('menu.customers_list')}}</li>
                    </ul>
                </div>
                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-header d-flex align-items-center">
                                <h3 class="h4">{{$trans('menu.customers_list')}}</h3>
                            </div>
                            <div class="">
                                <div class="card-body">
                                    ...
                                    <v-table
                                        ...
                                        <tr v-for="row in displayData" :key="row.traccar.id" :id="row.traccar.id">
                                            <td scope="row"><input type="checkbox" value=""></td>
                                            <td>{{row.traccar.name}}</td>
                                            <td>{{ row.traccar.email }}</td>
                                            <td v-if="row.traccar.administrator === true">{{$trans('main.yes')}}</td> <td v-else>{{$trans('main.no')}}</td>
                                            <td>{{row.eis.start_date | moment("DD/MM/YYYY")}}</td>
                                            <td>{{ row.traccar.expirationTime | moment("DD/MM/YYYY") }}</td>
                                            <td v-if="row.traccar.disabled === false"><span class="fas fa-check-circle text-success mr-2"></span>{{$trans('main.active')}}</td>
                                            ...
                                            <td style="overflow-y:visible !important">
                                                <a href="#" v-if="row.traccar.id !== connected.id" id="action_menu" data-toggle="dropdown" class="mr-2 fw-bold"><span
                                                ...
                                                <div aria-labelledby="action_menu" class="dropdown-menu">
                                                    ...
                                                    <router-link class="dropdown-item" :to="{name:'users-details', params: { idUser:row.traccar.id } }"><span class="fas fa-eye mr-2"></span>{{$trans('main.view_details')}}</router-link>
                                                    <router-link v-if="row.id !== connected.id" class="dropdown-item" :to="{name:'users-edit', params: { idUser:row.traccar.id } }"><span class="fas fa-edit mr-2"></span>{{$trans('main.edit')}}</router-link>
                                                    <a v-if="row.traccar.disabled === false" class="dropdown-item text-danger" @click="modalDisableUser(row)"><span class="fas fa-user-times mr-2"></span>{{$trans('main.suspend')}}</a>
                                                    <a v-if="row.traccar.disabled === true" class="dropdown-item text-soft-green" @click="modalEnableUser(row)"><span class="fas fa-user mr-2"></span>{{$trans('main.enable')}}</a>

                                                </div>

                                                <a v-if="row.traccar.id !== connected.id" @click="modalDeleteUser(row.traccar.id)">
...
export default {
    data () {
        return {
            ...
            totalPages: 0,
            filters: {
                search: { value: '', keys: ['traccar.name','traccar.email','traccar.expirationTime'] }
            }
        }
    },
    async mounted () {
    ...
    methods: {
        ...
        enableUser(item){
            this.isLoad = true;
            axios.put('/user/'+item.traccar.id+'/action/E',{},{
                headers: this.tokenHeader
            }).then(({data}) => {
                this.isLoad = false;
                if(data.success === true){
                    item.traccar.disabled = false;
                    Swal.fire(
        ...
        disableUser(item){
            this.isLoad = true;
            axios.put('/user/'+item.traccar.id+'/action/D',{},{
                headers: this.tokenHeader
            }).then(({data}) => {
                this.isLoad = false;
                if(data.success === true){
                    item.traccar.disabled = true;
                    Swal.fire(
        ...
        {
            this.isLoad = true;
            axios.put('user/'+item.traccar.id+'/password/reset',{password:this.newPassword},{
                headers: this.tokenHeader
            }).then(({data}) => {
                this.isLoad = false;
                if(data.success === true){
