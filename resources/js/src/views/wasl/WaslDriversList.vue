            ...
            <div class="row">
                <div class="col-sm-12 col-md-6 col-lg-6">
                    <ul class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">{{$trans('wasl.drivers')}}</li>
                    </ul>
                </div>
                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-header d-flex align-items-center">
                                <h3 class="h4">{{$trans('wasl.drivers')}}</h3>
                            </div>
                            <div class="">
                                <div class="card-body">
                                    ...
                                    <v-table
                                        ...
                                        <tr>
                                            ...
                                            <th class="border-bottom">{{ $trans('wasl.criminalRecordStatus') }}</th>
                                            <th class="border-bottom">{{ $trans('wasl.registrationTime') }}</th>
                                        ...
                                        <tr v-for="row in displayData" :key="row.id" :id="row.traccar_driver_id">
                                            <td scope="row"><input type="checkbox" value=""></td>
                                            <td>{{row.wasl.referenceKey}}</td>
                                            <td>{{row.wasl.name}}</td>
                                            <td>{{row.wasl.criminalRecordStatus}}</td>
                                            <td>{{row.wasl.registrationTime}}</td>
                                            <td v-if="row.wasl.operatingCompanies[0].isDriverValid === true"><span class="font-weight-normal"><span class="fas fa-check-circle text-success mr-2"></span>{{$trans('main.active')}}</span></td>
                                            ...
                                            <td style="overflow-y:visible !important">
                                                ...
                                                <a @click="modalDeleteDriver(row.traccar_driver_id)">
...
export default {
    data () {
        return {
            ...
            totalPages: 0,
            filters: {
                search: { value: '', keys: ['wasl.referenceKey','wasl.name','wasl.criminalRecordStatus'] }
            },
            idUser:null,
        }
    },
    async mounted () {
        ...
        await this.getWaslDriversList()
    },
    methods: {

        async getWaslDriversList() {
            this.isLoad = true;
            let id = this.idUser;
            await axios.get('/wasl/'+id+'/drivers', {
