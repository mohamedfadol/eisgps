            ...
            <div class="row">
                <div class="col-sm-12 col-md-6 col-lg-6">
                    <ul class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><router-link :to="{name:'wasl-list'}">{{$trans('wasl.wasl')}}</router-link></li>
                        <li class="breadcrumb-item active">{{$trans('wasl.vehicles')}}</li>
                    </ul>
                </div>
                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-header d-flex align-items-center">
                                <h3 class="h4">{{$trans('wasl.vehicles')}}</h3>
                            </div>
                            <div class="">
                                <div class="card-body">
                                    ...
                                    <v-table
                                        ...
                                        <tr>
                                            ...
                                            <th class="border-bottom">{{ $trans('wasl.plate_number') }}</th>
                                            <th class="border-bottom">{{ $trans('wasl.status') }}</th>
                                            <th class="border-bottom">{{ $trans('wasl.receivedTime') }}</th>
                                            <th class="border-bottom">{{ $trans('wasl.status') }}</th>
                                            <th class="border-bottom">{{ $trans('wasl.reason') }}</th>
                                            <th class="border-bottom">{{$trans('main.action')}}</th>

                                        </tr>
                                        </thead>
                                        <tbody slot="body" slot-scope="{displayData}">
                                        <tr v-for="row in displayData" :key="row.id" :id="row.traccar_device_id">
                                            <td scope="row"><input type="checkbox" value=""></td>
                                            <td>{{row.wasl.referenceKey}}</td>
                                            <td>{{row.wasl.plate.number}} {{row.wasl.plate.rightLetter}} {{row.wasl.plate.middleLetter}} {{row.wasl.plate.leftLetter}}</td>
                                            <td>{{$trans('waslreason.'+row.wasl.vehicleLocationInformation.vehicleStatus)}}</td>
                                            <td>{{row.wasl.vehicleLocationInformation.receivedTime}}</td>
                                            <td v-if="row.wasl.operatingCompanies[0].isVehicleValid === true"><span class="font-weight-normal"><span class="fas fa-check-circle text-success mr-2"></span>{{$trans('main.active')}}</span></td>
                                            <td  v-else><span class="font-weight-normal"><span class="fas fa-times-circle text-danger ml-2"></span> {{$trans('main.inactive')}}</span></td>
                                            <td><span class="font-weight-normal">{{$trans('waslreason.'+row.wasl.operatingCompanies[0].vehicleRejectionReason+'')}}</span></td>
                                            <td style="overflow-y:visible !important">
                                                ...
                                                <a  @click="modalDeleteCar(row.traccar_device_id)">
...
export default {
    data () {
        return {
            ...
            totalPages: 0,
            filters: {
                search: { value: '', keys: ['wasl.referenceKey','wasl.vehicleLocationInformation.vehicleStatus'] }
            },
            idUser:null,
        }
    },
    async mounted () {
        ...
        await this.getWaslVehiclesList()
    },
    methods: {

        async getWaslVehiclesList() {
            this.isLoad = true;
            let id = this.idUser;
            await axios.get('/wasl/'+id+'/vehicles', {
