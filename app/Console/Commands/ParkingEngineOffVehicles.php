...
class ParkingEngineOffVehicles extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'wasl:parked_off';
     ...
     * @var string
     */
    protected $description = 'Sent locations of all Parking and engine off vehicles every 60 minutes';
    ...
    public function handle()
    {
        try {
            ...
            foreach ($allVehicles as $vehicle){
                ...
                if($ignition == false && $motion == false){
            ...
            if(!empty($allLocations)){
                ...
                Log::channel('liveTracking')->info("----------LOCATION---PARKED---ENGINE--OFF---");
                ...
                Log::channel('liveTracking')->info("----------RESPONSE--LOCATION---PARKED---ENGINE--OFFG----------");
                Log::channel('liveTracking')->info(json_encode($dataVehLocation));
                Log::channel('liveTracking')->info("--------END---LOCATION---PARKED---ENGINE--OFF--------------");
            }
            $this->info('Successfully sent all parking and engine off vehicles to Wasl.');
        }catch (\Throwable $exception){
            Log::channel('liveTracking')->info("--------ERROR-ON--PARKED---ENGINE--OFF-------------");
