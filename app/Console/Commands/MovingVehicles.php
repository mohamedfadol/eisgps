...
class MovingVehicles extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'wasl:moving';
     ...
     * @var string
     */
    protected $description = 'Sent locations of all Moving vehicles every 2 minutes';
    ...
    public function handle()
    {
        try {
            ...
            if(!empty($allLocations)){
                ...
                Log::channel('liveTracking')->info("----------LOCATION---MOVING--------");
                ...
                Log::channel('liveTracking')->info("----------RESPONSE--LOCATION---MOVING----------");
                Log::channel('liveTracking')->info(json_encode($dataVehLocation));
                Log::channel('liveTracking')->info("--------END---LOCATION---MOVING--------------");
            }


            $this->info('Successfully sent all moving vehicles to Wasl.');
        }catch (\Throwable $exception){
            Log::channel('liveTracking')->info("--------ERROR-ON-MOVING--------------");
